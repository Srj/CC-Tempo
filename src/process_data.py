...
def read_data(args):
    ...
    cc = [torch.from_numpy(cc[(meta[args.tp_col] == d),:]).float() for d in y]

    return expr, x_, xp_, xu_, y, pca, um, tps, celltype, genes, cc
...
def create_parser():
    ...
    parser.add_argument('-c', '--cellcell', type=str, required=True, 
                    help='Path to the cell cell score file')
    # column names
    parser.add_argument('--tp_col', type=str, default='Time point', required=False,
...
def main(args):
    ...
    expr, x, xp, xu, y, pca, um, tps, celltype, genes, cc = read_data(args)
    ...
    torch.save({
     ...
     "w":w,
     "cc":cc
     }, os.path.join(args.out_dir,"data.pt"))
